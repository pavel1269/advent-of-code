...
function Calc-Map {
    ...
    $Wire1 = Parse-Wire $Wire1
    $Wire2 = Parse-Wire $Wire2

    $map = @{ Crossings = @()}

    Traverse-Wire $map $Wire1 "Wire1"
    Traverse-Wire $map $Wire2 "Wire2"
...
function Parse-Wire {
    ...
    return $Wire.Split(',') | ForEach-Object { @{
            Direction = $_[0]
            Length    = [int][string]::Join("", $_[1..$_.Length])
        } }
}

function Traverse-Wire {
    param($map, $Wire, $Name)
    ...
    $Wire | ForEach-Object {
        ...
        $dirData = switch ($coord.Direction) {
            'R' { @{ x = 1; y = 0 } }
            'L' { @{ x = -1; y = 0 } }
            'U' { @{ x = 0; y = 1 } }
            'D' { @{ x = 0; y = -1 } }
            default {
                throw "Unown direction '$($coord.Direction)'"
            }
        }

        for ($length = 0; $length -lt $coord.Length; $length++) {
            $pos.x += $dirData.x
            $pos.y += $dirData.y
            $distance++
            ...
            if ($map.$($pos.x)) {
                if ($map.$($pos.x).$($pos.y)) {
                    $map.$($pos.x).$($pos.y).$Name = $true

                    if (-not $map.$($pos.x).$($pos.y).$($Name + "Distance")) {
                        $map.$($pos.x).$($pos.y).$($Name + "Distance") = $distance
                    }

                    if ($map.$($pos.x).$($pos.y).Wire1 -and $map.$($pos.x).$($pos.y).Wire2) {
                        $map.Crossings += @{
                            x = $pos.x
                            y = $pos.y
                            Distances = $map.$($pos.x).$($pos.y).Wire1Distance + $map.$($pos.x).$($pos.y).Wire2Distance
                        }
                    }
                }
                else {
