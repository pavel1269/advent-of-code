...
pub fn create_image(input: &str) -> Vec<String> {
    ...
    for index_y in 0..map_dimension {
        for index_x in 0..map_dimension {
            ...
            for index_map_y in 1..real_tile_dimension {
                if index_x == 0 {
                    image_rows.push(String::with_capacity(map_dimension * real_tile_dimension));
                }
                let slice = &map_rows[index_map_y][1..tile_dimension - 1];
                image_rows[index_y * real_tile_dimension + index_map_y - 1 - index_y]
                    .push_str(slice);
            }
        }
    }
